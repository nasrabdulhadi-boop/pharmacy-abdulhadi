...
BEGIN
 ...
 SELECT jsonb_build_object(
  ...
  'items',COALESCE((SELECT jsonb_agg(to_jsonb(x) ORDER BY x.sales DESC) FROM (SELECT p.name AS product_name,SUM(si.quantity) AS quantity,SUM(si.quantity*si.unit_price) AS sales,SUM(si.quantity*si.unit_cost) AS cost,SUM(si.quantity*(si.unit_price-si.unit_cost)) AS profit FROM public.sale_items si JOIN public.products p ON p.id=si.product_id WHERE si.created_at>=p_start AND si.created_at<p_end GROUP BY p.id,p.name ORDER BY SUM(si.quantity*si.unit_price) DESC LIMIT 50) x),'[]'::jsonb)
 ) INTO v;
 RETURN v;
END;
$$;
