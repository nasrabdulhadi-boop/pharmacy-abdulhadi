...
AS $$
DECLARE
  sale_id uuid := gen_random_uuid(); it jsonb; b record; need numeric; take numeric;
...
BEGIN
  ...
  FOR it IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    FOR b IN SELECT id,quantity,purchase_price,expiry_date FROM public.batches WHERE product_id=v_product_id AND quantity>0 AND (expiry_date IS NULL OR expiry_date>=current_date) ORDER BY expiry_date NULLS LAST,received_date NULLS FIRST,id FOR UPDATE LOOP
      EXIT WHEN need<=0; take:=least(need,b.quantity);
      UPDATE public.batches SET quantity=quantity-take WHERE id=b.id;
      INSERT INTO public.sale_items(sale_id,product_id,batch_id,quantity,unit_price,unit_cost) VALUES(sale_id,v_product_id,b.id,take,v_unit_price,b.purchase_price);
      INSERT INTO public.stock_movements(product_id,batch_id,movement_type,quantity,reference_id,created_by) VALUES(v_product_id,b.id,'sale',-take,sale_id,auth.uid());
