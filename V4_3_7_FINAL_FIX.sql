...
AS $$
DECLARE
  v_sale_id uuid := gen_random_uuid(); it jsonb; v_batch record; need numeric; take numeric;
...
BEGIN
  ...
  VALUES(v_sale_id,p_customer_id,0,greatest(coalesce(p_discount,0),0),0,auth.uid());
  FOR it IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    FOR v_batch IN SELECT bt.id,bt.quantity,bt.purchase_price,bt.expiry_date FROM public.batches AS bt WHERE bt.product_id=v_product_id AND bt.quantity>0 AND (bt.expiry_date IS NULL OR bt.expiry_date>=current_date) ORDER BY bt.expiry_date NULLS LAST,bt.received_date NULLS FIRST,bt.id FOR UPDATE OF bt LOOP
      ...
      INSERT INTO public.sale_items(sale_id,product_id,batch_id,quantity,unit_price,unit_cost) VALUES(v_sale_id,v_product_id,v_batch.id,take,v_unit_price,v_batch.purchase_price);
      INSERT INTO public.stock_movements(product_id,batch_id,movement_type,quantity,reference_id,created_by) VALUES(v_product_id,v_batch.id,'sale',-take,v_sale_id,auth.uid());
  ...
  UPDATE public.sales SET subtotal=sub,total=v_total WHERE id=v_sale_id;
  ...
  INSERT INTO public.audit_logs(user_id,action,entity_type,entity_id,details)
  VALUES(auth.uid(),'complete_sale','sale',v_sale_id,jsonb_build_object('subtotal',sub,'discount',p_discount,'total',v_total,'items',v_sale_details));
  RETURN v_sale_id;
END;
$$;
